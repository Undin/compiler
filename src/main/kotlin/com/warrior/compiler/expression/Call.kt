...
class Call(ctx: ParserRuleContext, val fnName: String, val args: List<Expr>) : Expr(ctx) {
    override fun generateCode(module: LLVMModuleRef, builder: LLVMBuilderRef,
        ...
        return generate(fnName, args, type, module, builder, symbolTable)
    }

    override fun determineTypeInternal(functions: Map<String, Type.Fn>, variables: SymbolTable<Type>): Type {
        return functions[fnName]?.returnType ?: Unknown
    }
...
class ExtensionCall(ctx: ParserRuleContext, val objectExpr: Expr, val fnName: String, val args: List<Expr>) : Expr(ctx) {
    override fun generateCode(module: LLVMModuleRef, builder: LLVMBuilderRef, symbolTable: SymbolTable<LLVMValueRef>): LLVMValueRef {
        val allArgs = listOf(objectExpr) + args
        return generate(fnName, allArgs, type, module, builder, symbolTable)
    }

    override fun validate(functions: Map<String, Type.Fn>, variables: SymbolTable<Type>): Result {
        var result = objectExpr.validate(functions, variables)
        result += args.map { it.validate(functions, variables) }.fold()
        ...
        if (fnType == null) {
            ...
            val argsTypes = args.map { it.determineType(functions, variables) }
            ...
            if (!fnType.isExtension) {
                ...
                if (args.size + 1 != fnType.argsTypes.size) {
                    val message = "'${getText()}': you must pass ${fnType.argsTypes.size - 1} args to function '$fnName'"
                    result += Error(ErrorMessage(WRONG_ARGS_NUMBER, message, start(), end()))
                }
                val minSize = Math.min(args.size, fnType.argsTypes.size - 1)
    ...
    override fun calculate(functions: Map<String, Fn>, variables: Map<String, TypedValue>): TypedValue {
        val fn = functions[fnName]!!
        val arguments = (listOf(objectExpr) + args).map { it.calculate(functions, variables) }.toList()
        return fn(arguments)
    }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return fnName == other.fnName && objectExpr == other.objectExpr && args == other.args
    }
    ...
    override fun toString(): String = "$objectExpr.$fnName(${args.joinToString()})"
}

private fun generate(fnName: String, args: List<Expr>, type: Type,
                     module: LLVMModuleRef, builder: LLVMBuilderRef,
                     symbolTable: SymbolTable<LLVMValueRef>): LLVMValueRef {
